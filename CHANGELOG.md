...
- Skip standalone `AddForeignKey`/`DropForeignKey` operations on SQLite, which has no `ALTER TABLE` syntax to add or drop a foreign key (such changes go through the existing table rebuild)
